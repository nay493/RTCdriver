...
/*ASSUME RTC_IRQ IS DEFINED HERE IN OUR CASE*/
...
static int rtc_do_ioctl(unsigned int cmd, unsigned long arg, int kernel)
{
...
#ifdef RTC_IRQ
	if (rtc_has_irq == 0) { //nay@ if rtc_has_irq == 0 then go to corresponding commnad of ioctl and do nothing ???
		switch (cmd) {
		case RTC_AIE_OFF:
		case RTC_AIE_ON:
		case RTC_PIE_OFF:
	...
	{
		mask_rtc_irq_bit(RTC_AIE);//here mask means to disable this bit ??
		return 0;
	}
	...
	case RTC_ALM_SET:	/* Store a time into the alarm */
	{
		...
		if (!(CMOS_READ(RTC_CONTROL) & RTC_DM_BINARY) ||
			...
			if (sec < 60)
				...
				sec = 0xff;		//Writing 0xff means "don't care" or "match all"
	...
	{
		memset(&wtime, 0, sizeof(struct rtc_time));//clear wtime
	/*The C library function void *memset(void *str, int c, size_t n) copies the character c (an unsigned char) to the first n characters 			of the string pointed to by the argument str.*/
		rtc_get_rtc_time(&wtime);//get wtime
		break;
	}
...
would succeed if either of the names __alpha__ or __mips__ is defined as a macro.used to check if multiple macros are defined 
*/
